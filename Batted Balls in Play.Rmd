...
```{r}
pitchingStats <- pitchingStats %>% filter(!is.na(vFA..pi.)) %>% filter(IP > 100) %>% select(-c(Dollars, playerid))
```
...
```{r}
model <- randomForest(ERA ~ ., data = train_set, ntree = 50)
```

```{r}
baseImp <- importance(model)
baseImp
```
...
```{r}
plot(model, col = 'orange', main = 'Base Model Error')
```

```{r}
newModel <- randomForest(ERA ~ SO + FB + GB + LD + H + HR + BB, data = train_set, ntree = 50)
```

```{r}
ERA_train <- predict(newModel, train_set, type = "response")
ERA_test <- predict(newModel, test_set, type = "response")
```
...
```{r}
ERA_Full <- rbind(train_set, test_set)
```

```{r}
plot(newModel, col = "orange", main = "New Model Error Chart")
```
...
```{r}
ERA_Full_1 <- ERA_Full %>% mutate(ERA_Diff = ERAPred - ERA) %>% select(c(Name, ERA, ERAPred, ERA_Diff, SO, GB, FB, LD, BB, H, HR))
```
